...
# ////////////////////////////////////////////////////////////////////////////////////////////////////////////
# //// PROBLEMA DE CLASIFICACIÓN DE EDAD CON RADIOGRAFÍA MAXILOFACIAL
# ////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#
SET_PRED_METHODS = ['base', 'LAC', 'MCM', 'APS', 'RAPS', 'SAPS']
CONFORMAL_PRED_METHODS = ['LAC', 'MCM', 'APS', 'RAPS', 'SAPS']
...
class MaxillofacialXRayDataset(Dataset):
    ...
    def __init__(self, metadata_file, images_dir, transform=None):
        ...
        # Edad en entero
        self.int_ages = metadata['Age'].astype(int)
    ...
    def __init_classes__(self):
        ...
        unique_ages = sorted(self.int_ages.unique())
        age_to_idx = {age: idx for idx, age in enumerate(unique_ages)}
        
        # Mapear cada edad entera a su índice de clase
        self.labels = torch.tensor(self.int_ages.map(age_to_idx).values, dtype=torch.long)
...
if args.test:
    ...
    if pred_method == 'base':   
        #
        test_pred_labels, test_pred_sets, test_true_labels = model.inference(test_loader, confidence)
    ...
    if args.save_test_results:
        ...
        if os.path.exists(args.save_test_results):
            existing_df = pd.read_csv(args.save_test_results, dtype={
                ...
                "iteration": np.int64,
                "pred_class": np.uint8,
                "pred_set_14": np.uint8,
                "pred_set_15": np.uint8,
                "pred_set_16": np.uint8,
                "pred_set_17": np.uint8,
                "pred_set_18": np.uint8,
                "pred_set_19": np.uint8,
                "pred_set_20": np.uint8,
                "pred_set_21": np.uint8,
                "pred_set_22": np.uint8,
                "pred_set_23": np.uint8,
                "pred_set_24": np.uint8,
                "pred_set_25": np.uint8,
                "pred_set_26": np.uint8,
                "true_class": np.uint8,
                "sex": str
            })
