...
# ////////////////////////////////////////////////////////////////////////////////////////////////////////////
# //// PROBLEMA DE CLASIFICACIÓN DE SEXO CON RADIOGRAFÍA MAXILOFACIAL
# ////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
# junto con su sexo correspondiente desde un fichero de metadatos. 
class MaxillofacialXRayDataset(Dataset):
    ...
    def __init_classes__(self):
        ...
        # Clasificación binaria de sexo: '0' para masculino y '1' para femenino
        self.labels = self.sexes.map({'M': 0, 'F': 1}).astype('int').values  # convierte a array de NumPy
        self.labels = torch.tensor(self.labels, dtype=torch.long)           # convierte a Tensor de PyTorch
...
#-------------------------------------------------------------------------------------------------------------
    ...
    if args.save_test_results:
        ...
        new_df = pd.DataFrame({
            ...
            "pred_sex": np.where(np.array(test_pred_classes, dtype=np.uint8) == 1, "F", "M"),
            "pred_set_male": np.array(test_pred_sets[:, 0], dtype=np.uint8),
            "pred_set_female": np.array(test_pred_sets[:, 1], dtype=np.uint8),
        })
        ...
        if os.path.exists(args.save_test_results):
            existing_df = pd.read_csv(args.save_test_results, dtype={
                ...
                "sex": str,
                "pred_sex": str,
                "pred_set_male": np.uint8,
                "pred_set_female": np.uint8
            })
