...
INTERVAL_PRED_METHODS = ['base', 'ICP', 'QR', 'CQR']
CONFORMAL_PRED_METHODS = ['ICP', 'CQR']

# Agregado de argumentos 
def add_model_args(parser):
    parser.add_argument('--load_model_path', type=str)
    parser.add_argument('--save_model_path', type=str)
    parser.add_argument('--pred_method', type=str, choices=INTERVAL_PRED_METHODS)
...
if args.pred_method in ['base','QR']:
...
if pred_method not in INTERVAL_PRED_METHODS:
    ...
    if args.save_test_results is not None:
        ...
        if os.path.exists(args.save_test_results):
            ...
            mask = ~(
                ...
                (existing_df["iteration"] == test_iteration)
            )
...
#         print(f"Predicción puntual para {os.path.basename(img_path)} con {pred_method}: {pred.item():.3f}")
...
#         print(f"Intervalo para {os.path.basename(img_path)} con {pred_method}: "+
