...
% La comparativa entre métodos se realizará unica y exclusivamente sobre el conjunto test, para garantizar que 
% la evaluación sea objetiva y no esté influenciada por ninguna etapa del entrenamiento o calibración.
...
Para cada método se ha entrenado 10 modelos independientes desde cero, con el objetivo de capturar la 
...
Dado que las imágenes del conjunto de datos disponible son significativamente más anchas que altas, se 
...
\begin{itemize}
    \item Error cuadrático medio como función de pérdida en modelos de predicción puntual y 
    \textit{pinball loss} para modelos QR. 

    El error cuadrático medio es la función de pérdida por defecto para problemas de regresión: 
    los errores siguen una distribución normal, lo que hace que minimizar el MSE equivalga a maximizar
    la verosimilitud de los datos;
    penaliza los errores grandes más que los pequeños, lo que ayuda a evitar predicciones extremadamente 
    alejadas de los valores reales;
    y es derivable en todo su dominio, ---además de que su derivada es lineal, lo que facilita el cálculo 
...
\todo{También podría reformular este apartado y llamarlo `Evaluación de los experimentos', e incluir tanto
métricas como las tests estadísticos que empleo en experimentación.}
...
\todo{¿Debería indicar los supuestos a la hora de usar ANOVA y Tukey? Por ejemplo: se asume que cada método
presenta distribución normal en las métricas}
...
\begin{table}[h]
    ...
    \end{tabular}
    \caption[
        Cobertura empírica y amplitud media del intervalo de predicción obtenidos por cada método de predicción a lo 
        largo de distintas ejecuciones.
    ]{   
        Cobertura empírica y amplitud media del intervalo de predicción obtenidos por cada método de predicción a lo 
        largo de distintas ejecuciones. Se presentan los valores para cada ejecución individual, así como la 
...
En contraste, los métodos conformales (ICP y CQR) sí logran coberturas próximas al valor nominal, tal como
se espera dada su fundamentación estadística. Esta mayor cobertura, sin embargo, tiene un costo en cuanto
a la amplitud del intervalo, que tiende a ser mayor que en los métodos conformales.
Esta relación de compromiso o \textit{trade-off} entre cobertura y amplitud de los intervalos ---típico en
la predicción interválica--- se visualiza claramente en la Figura \ref{fig:AE_scatterplot_EC-MPIW}, donde se 
observa una alta correlación entre la cobertura empírica y el tamaño del intervalo de predicción.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{capitulos/cap_05/imagenes/AE_scatterplot_EC-MPIW.png}
    \caption[
        Gráfica de dispersión \textit{Empirical Coverage}-\textit{Mean Interval Width}
    ]{
        Gráfica de dispersión \textit{Empirical Coverage}-\textit{Mean Interval Width}. 
        Elaboración propia. 
...
Sin embargo, CQR presenta unas amplitudes promedias de intervalo significativamente más reducidas que ICP, 
...
\todo{Apliqué ANOVA y Tukey aquí, pero no resultaron muy útiles ya que no tienen en cuenta la relación de
correlación entre cobertura empírica y tamaño medio del intervalo, y resultaba en que ICP y CQR no mostraban
coberturas significativamente diferentes, lo que es verdad, pero no presenta una información completa. 
¿Merece la pena hacer MANOVA (ANOVA para múltiples variables) o directamente con la figura ya queda claro?}
...
\todo{Esto ocurre, pero no sé por qué. Voy a investigar, pero me temo que va a ser difícil hallar la razón,
ya que muy probablemente sea fruto del funcionamiento de la red en regresión cuantílica, y al ser un modelo
de caja negra, no pueda hacer nada.}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.8\textwidth}
        ...
        \caption{
            Histograma de amplitud del intervalo de predicción con diferenciación por cobertura 
            (modelo CQR).
        }
...
\todo{No estoy entrando a hacer valoraciones de lo grave o leve que sea que la cobertura se reduzca de un 95 
a un 93.4, porque entiendo que aquí entraría mi subjetividad, y esa debe ir más en las conclusiones que aquí, 
¿correcto?}
...
\begin{table}[]
    ...
    \toprule
    \multirow{2}{*}{\textbf{Método}} &  & \multicolumn{4}{c}{\textbf{Cobertura Empírica (\%)}}      &  & \multicolumn{4}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Amplitud Media \\ del Intervalo\end{tabular}}} \\ \cline{3-6} \cline{8-11} 
                                    &  & \textbf{base} & \textbf{ICP} & \textbf{QR} & \textbf{CQR} &  & \textbf{base}            & \textbf{ICP}            & \textbf{QR}           & \textbf{CQR}           \\ \cline{1-1} \cline{3-6} \cline{8-11} 
...
% \hline
% Método       & \multicolumn{3}{c|}{base} & \multicolumn{3}{c|}{LAC} & \multicolumn{3}{c|}{MCM} \\ \hline
% Métrica      & EC      & MPSS   & MSS    & EC      & MPSS   & MSS   & EC      & MPSS   & MSS   \\ \hline
% Unidad       & \%      & —      & —      & \%      & —      & —     & \%      & —      & —     \\ \hline
% Ejecución 1  & 87.87   & 1      & 3.43   & 94.80   & 1.20   & 2.25  & 93.91   & 1.19   & 2.41  \\ 
% Ejecución 2  & 87.87   & 1      & 3.43   & 95.07   & 1.20   & 2.19  & 94.38   & 1.21   & 2.33  \\ 
% Ejecución 3  & 87.59   & 1      & 3.48   & 95.12   & 1.23   & 2.20  & 94.24   & 1.23   & 2.38  \\ 
% Ejecución 4  & 87.59   & 1      & 3.48   & 93.96   & 1.19   & 2.40  & 94.42   & 1.21   & 2.33  \\ 
% Ejecución 5  & 87.64   & 1      & 3.47   & 94.05   & 1.18   & 2.37  & 93.54   & 1.19   & 2.48  \\ 
% Ejecución 6  & 87.27   & 1      & 3.55   & 93.96   & 1.18   & 2.38  & 93.63   & 1.20   & 2.47  \\ 
% Ejecución 7  & 88.06   & 1      & 3.39   & 94.10   & 1.19   & 2.37  & 93.87   & 1.20   & 2.42  \\ 
% Ejecución 8  &         &        &        &         &        &       &         &        &       \\ 
% Ejecución 9  &         &        &        &         &        &       &         &        &       \\ 
% Ejecución 10 &         &        &        &         &        &       &         &        &       \\ \hline
% \rowcolor[HTML]{EFEFEF} 
% MEDIA        & 87.7    & 1      & 3.46   & 94.44   & 1.2    & 2.31  & 94      & 1.2    & 2.4   \\ \hline
% \end{tabular}
% \caption[
%     Resultados de las predicciones obtenidas por los modelos para el problema de estimación de mayoría de 
%     edad en cada ejecución.
% ]{   
%     Resultados de las predicciones obtenidas por los modelos para el problema de estimación de mayoría de 
%     edad en cada ejecución.
%     Punto como separador decimal.
%     Leyenda: EC (Empiricial Coverage), MPSS (Mean Prediction Set Size), MSS (Mean Set Score).
% }
% \label{tab:comparativa_conformal_AMM}
% \end{table}
...
%\subsection{Resultados para la clasificación combinada de sexo y edad}
