...
Por ello, la función de \acrshort{NC} es el error absoluto de la predicción respecto al valor real:
...
Luego, el umbral de \acrshort{NC} para un nivel de confianza $1-\alpha$ se calcula como el cuantil $(1-\alpha)(1+1/n)$ de las puntuaciones de \acrshort{NC}:
...
\begin{itemize}
    ...
    \item \textbf{Bajo coste computacional}: El método introduce una fase adicional tras el entrenamiento, denominada calibración, en la que se calculan las puntuaciones de \acrshort{NC} sobre el conjunto de calibración $\left( \mathcal{O}(n_{calib}) \right)$ y se determina el umbral correspondiente mediante el cálculo de cuantiles $\left( \mathcal{O}(n_{calib} \log n_{calib}) \right)$. En consecuencia, el coste global es $\mathcal{O}(n_{calib} \log n_{calib})$. Aun cuando este orden de complejidad es superior al lineal, en la práctica el tiempo requerido resulta despreciable en comparación con el entrenamiento de modelos complejos, como \textit{ensembles} o \acrshort{DNN}. La inferencia conformal no introduce ningún cambio de orden ($\mathcal{O}(1)$), ya que tan solo calcula la puntuación de \acrshort{NC} de la nueva instancia para compararla con el umbral. 
...
\begin{itemize}
    \item Se calculan las puntuaciones de \acrshort{NC} sobre los datos del conjunto de calibración como las diferencias entre los valores observados y los límites del intervalo predictivo:
    ...
    \item Se calcula un umbral de \acrshort{NC} para un nivel de confianza dado $1-\alpha$ como el cuantil $(1-\alpha)(1+1/n)$ de $R$:
...
\acrshort{CQR}, al igual que \acrshort{ICP}, es \textbf{independiente del modelo y del dominio}, ya que solo emplea las salidas y valores reales para realizar la calibración%
\footnote{
    Que sea independiente del modelo no significa que no requiera reentrenamiento. CQR requiere de la adaptación de modelos de una sola salida para que tengan las tres salidas cuantílicas. 
}
y no usa representaciones de las entradas. También tiene un \textbf{similar coste computacional}, puesto que realiza prácticamente las mismas operaciones que \acrshort{ICP}, pero para cada límite del intervalo predicho, calibrando los cuantiles inferior y superior de manera independiente para mantener la cobertura deseada. Sin embargo, \acrshort{CQR} logra \textbf{intervalos asimétricos y adaptativos}, dado que la regresión cuantílica estima directamente los cuantiles condicionales de la distribución de la variable objetivo, permitiendo que los límites del intervalo se ajusten según la heterocedasticidad y la forma local de la distribución de los datos, en lugar de asumir una distribución simétrica o constante del error. 
...
\begin{table}[h]
    ...
    %
    \begin{tabular}[c]{@{}c@{}}Cobertura marginal\\[-1.2ex]garantizada\end{tabular} & Sí & No & Sí \\
    %
    \begin{tabular}[c]{@{}c@{}}Cobertura condicional\\[-1.2ex]garantizada\end{tabular} & No & No & No \\
    %
    ...
    \caption[
        ...
        Comparativa de métodos propuestos para problemas de regresión. Las técnicas de \acrshort{CP} son las únicas que garantizan cobertura marginal.
    }
...
El conjunto de predicción conformal de una nueva instancia $x_{n+1}$ se construye como las clases cuyas probabilidades superan la unidad menos el umbral de \acrshort{NC} (véase la Figura \ref{fig:LAC_threshold}):
...
\begin{itemize}
    ...
    \item \textbf{Bajo coste computacional}: Al igual que pasaba en regresión, solo añade coste computacional en la calibración, con el cálculo de puntuaciones de \acrshort{NC} ($\mathcal{O}(n_{calib})$) y la obtención del umbral de \acrshort{NC} ($\mathcal{O}(n_{calib} \textnormal{log } n_{calib})$). No añade coste a la inferencia ($\mathcal{O}(1)$).
...
\begin{figure}[h]
    ...
    \caption[
        ...
        Diagrama ilustrativo de la división de ejemplos utilizada en \acrshort{MCM}. Para cada clase se calcula un umbral de \acrshort{NC} a partir de las puntuaciones de no conformidad de los ejemplos pertenecientes a esa clase.
    }
...
\begin{itemize}
    ...
    \item \textbf{Coste computacional comparable a \acrshort{LAC}}: Al igual que LAC, MCM calcula las puntuaciones de \acrshort{NC} para todas las instancias del conjunto de calibración. Posteriormente, para cada clase se ordenan las puntuaciones de su subconjunto para determinar el umbral correspondiente. Si hay $K$ clases y $n_k$ instancias por clase, el coste total de esta fase es $\sum_{k=1}^{K} \mathcal{O}(n_k \log n_k)$, que en la práctica se aproxima a $\mathcal{O}(n_{calib} \log n_{calib})$, manteniendo la complejidad algorítmica de LAC. En la fase de inferencia, la predicción para una nueva instancia requiere únicamente calcular la puntuación de \acrshort{NC} y compararla con el umbral de su clase, lo que supone un coste lineal respecto al número de clases, $\mathcal{O}(K)$, que se sigue traduciendo en tiempos prácticamente despreciables para la inmensa mayoría de problemas. Por tanto, MCM sigue siendo eficiente.
...
Una extensión útil de \acrshort{APS} consiste en introducir una componente aleatoria durante la fase de calibración para compensar la tendencia del método a la \textit{sobrecobertura}. En este ajuste, cuando se calcula la puntuación de \acrshort{NC} para una instancia de calibración, se decide excluir la contribución de la última clase incluida (aquella correspondiente a la etiqueta verdadera $y_i$) con una probabilidad proporcional al exceso de cobertura ---respecto al nivel de confianza $1-\alpha$--- que dicha clase genera.
...
\begin{itemize}
    ...
    \item El \textbf{coste computacional resulta más elevado que los dos algoritmos previos}. Durante la fase de calibración, cada ejemplo requiere ordenar las probabilidades de las clases para calcular su puntuación de no conformidad, lo que implica un coste total de $\mathcal{O}(n_{calib} \cdot K \log K)$ donde $K$ es el número total de clases. En la fase de inferencia, el procedimiento se repite para cada nueva instancia: se ordenan las clases por probabilidad y se van acumulando hasta superar el cuantil de calibración, alcanzando un coste de $\mathcal{O}(K \log K)$ por predicción. Aunque este orden es superior al del método clásico, en la práctica sigue siendo asumible para valores moderados de $K$.
...
A diferencia de los anteriores métodos adaptativos, este método incluye la \textbf{componente aleatoria} desde su planteamiento inicial. La función de \acrshort{NC} propuesta es:
