...
Una vez hemos definido la arquitectura a emplear para resolver un problema, debemos entrenar la red con los 
datos de ejemplo. Este proceso implica ajustar los parámetros del modelo (los pesos y sesgos) para minimizar
...
\textit{backpropagation}}, que funciona en dos fases clave:
...
\begin{itemize}
    ...
    para reducir el error en futuras iteraciones.
...
\begin{itemize}
    ...
    entrenamiento de una red neuronal, que cumple tres requisitos clave:
    ...
    \begin{enumerate}
        ...
        \item Ser diferenciable: Es esencial para aplicar técnicas de descenso por gradiente (como 
        \textit{backpropagation}), ya que el optimizador necesita calcular derivadas.
    ...
    incorrectas y ayuda a optimizar probabilidades predichas.

    \item Si bien existen métodos de entrenamiento de redes ejemplo a ejemplo ---como el Gradiente Descendente 
    Estocástico puro, SGD con tamaño de lote 1---, estas se suelen entrenar por lotes (\textit{batches}) 
    debido a ventajas clave, como el aprovechamiento de la paralelización de operaciones en GPU, la reducción 
    de la sobrecarga de comunicación entre CPU y GPU, o la evitación de oscilaciones bruscas en las funciones 
    de pérdidas (al ser el error promediado entre el número de ejemplos por lote). Aún así, establecer un 
    tamaño de lote óptimo es una tarea no trivial que requiere de encontrar un equilibrio entre generalización 
    y velocidad: los lotes pequeños introducen ruido que puede ayudar a escapar de mínimos locales; los lotes 
    grandes aceleran el entrenamiento pero pueden reducir la generalización \cite{keskar2017}.
...
: 
variantes de funciones de pérdida, múltiples algoritmos de de optimización de parámetros, elección de 
hiperparámetros, técnicas de regularización como early stopping o batch normalization, ...





¿Cómo se calcula el error? Mediante una \textbf{función de pérdida (\textit{loss function})}. 
 




Este proceso se repite durante múltiples \textbf{épocas} (qué es como se denomina a las iteraciones sobre
el conjunto de datos) hasta que el modelo alcanza un rendimiento satisfactorio o converge.





Para ello, son clave los siguientes componentes:

\begin{itemize}

    \item \textbf{Función de pérdida (\textit{loss function})}: 
    Esta función cuantifica el error del modelo durante el entrenamiento, midiendo la discrepancia entre las 
    predicciones generadas y los valores o clases reales (\textit{ground truth}).
    ...
    \item \textbf{Algoritmo de optimización de pesos}: 
    Estos algoritmos determinan cómo actualizar los parámetros del modelo durante el entrenamiento para 
    minimizar la función de pérdida. El más utilizado es el \textit{descenso de gradiente} (y sus variantes 
    como SGD, Adam o RMSprop), que ajusta los pesos en dirección opuesta al gradiente de la función de pérdida 
    respecto a los parámetros. La elección del optimizador afecta la velocidad de convergencia y la 
    capacidad de evitar mínimos locales.
...
\begin{itemize}
    ...
    \item el \textit{classifier}\footnote{
        Se le denomina así aunque resuelva problemas de clasificación o regresión.
    }, generalmente implementado mediante una o más capas totalmente conectadas, toma estas representaciones 
    para realizar la tarea específica de salida, ya sea clasificación o regresión.
...
\begin{itemize}
    ...
    Algunas alteraciones realizadas pueden ser: cambiar el nivel de brillo y contraste, rotar, trasladar, 
    escalar o voltear las imágenes, añadir ruido

    Esto sirve para problemas como la clasificación o regresión, donde las clases o valores predichos no
    suelen variar bajo pequeñas perturbaciones locales. 
    ...
    dependa demasiado de unidades individuales. En CNNs suele aplicarse a capas fully-connected, aunque 
    existen variantes como \textit{Spatial Dropout} \cite{tompson2015} que elimina canales completos 
    en capas convolucionales, forzando una distribución más robusta de características.
    
    \item \textbf{\textit{Batch normalization}}: Esta se introduce como una capa nueva a añadir en el diseño
    de las redes, con nuevos parámetros entrenables: \textit{scale} y \textit{shift}. 
    ...
    Esto estabiliza la dinámica del aprendizaje (\textit{internal covariate shift}), actúa como regularizador 
    suave al añadir ruido a las activaciones, y permite usar tasas de aprendizaje más altas. En CNNs se aplica 
    típicamente después de las capas convolucionales y antes de la función de activación
